...
struct HistoryPeriodView: View {
    @State var periodType: DateHelpers.TimeComponents = .week
    @State var currentPeriodStart: Date = Date.now
    @State var isConfirmingDelete = false
    @State var deleteIndices: IndexSet = []
    @State var sessionsShowing = false
    
    @State var nextPeriodStart: Date = Date.now
            ...
            TwoOptionView(isFirstShowing: $sessionsShowing) {
                List {
                    ForEach(daysOfWeek, id: \.self) {weekday in
                        Section(header: Text(DateHelpers.dateToWeekday(weekday))) {
    ...
    func update() {
        ...
        switch (periodType) {
        case .week:
            matchComponents = DateHelpers.FirstOfWeek
        case .month:
            matchComponents = DateHelpers.FirstOfMonth
        case .year:
        ...
        nextPeriodStart = Calendar(identifier: .gregorian).nextDate(after: Date.now, matching: matchComponents, matchingPolicy: .strict)!
